...
translationTests = testGroup "translationTests"
  ...
  , testCase "Letcase" $
    translateStr (PLetCase "y" (PProjector 1 (PQubits "1")) [PQubits "1",PQubits "0"]) @?= "letcase("<>qubit1Str<>".measure(0),[lambda : "<>qubit1Str<>",lambda : Circuit(1),])"
  , testCase "add one two qubits" $
    translateStr (PLambda "x" (PLetCase "y" (PProjector 1 (PVar "x")) [PQubits "1",PQubits "0"])) @?= "lambda x: letcase(x.measure(0),[lambda : "<>qubit1Str<>",lambda : Circuit(1),])"
  ]
  where
